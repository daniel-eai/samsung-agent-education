# Loop Engineering 실습 프롬프트

> 교육생 설정은 Step 1에서 한 번만 입력하고 MEMORY.md의 `Project Settings`에서 이후 Step들이 재사용합니다.
...
- 웹사이트 디자인 참고: [없음, 다크 테마, 메트릭스 느낌, 페이스북 등등]
- 게임 추가 기능: [없음 또는 원하는 기능 - 랜덤하게 움직이는 적]

[Step 1 - 개발 업무 분석]

먼저 위 [교육생 설정]을 MEMORY.md의 `Project Settings`에 기록해.
MEMORY.md가 없으면 해당 설정 섹션만 생성해.
토큰 값 자체는 읽거나 저장하지 말고 토큰 파일명만 기록해.
...
MEMORY.md의 `Project Settings`, STEP1_ANALYSIS.md와 저장소 구조를 읽고 실행 가능한 AORR 상태 머신을 설계해.
...
MEMORY.md의 `Project Settings`, AORR.md와 저장소 구조를 읽고 Verifier 중심 Self-Correcting TDD 루프를 설계해.
...
Step 1에서 생성한 MEMORY.md와 AORR.md를 읽고, `Project Settings`를 보존하면서 프로젝트 복구용 MEMORY.md를 완성해.
...
MEMORY.md의 `Project Settings`, STEP1_ANALYSIS.md, AORR.md와 기존 파일을 읽고 가장 안전한 첫 개발 루프를 정확히 1회 실행해.
...
MEMORY.md의 `Project Settings`, STEP1_ANALYSIS.md, AORR.md와 Step 5 결과를 읽고 자동화 준비도를 평가해.
...
MEMORY.md의 `Project Settings`, STEP1_ANALYSIS.md, AORR.md와 기존 코드를 읽고 작은 AORR·TDD 루프를 순서대로 실행해 전체 사이트를 완성해.
...
- 반대 방향 방지
- 중복 타이머 방지
- MEMORY.md의 `Project Settings`에 기록된 게임 추가 기능
...
MEMORY.md의 `Project Settings`, 배포된 사이트, 현재 코드, Git 상태, AORR.md,
...
MEMORY.md의 `Project Settings`, CHANGE_REQUEST.md, AORR.md, 현재 코드와
...
- game.js (있는 경우)
- assets/ (있는 경우)
- 기타 웹사이트 실행에 필요한 HTML, CSS, JavaScript 정적 파일

위 파일을 GitHub 저장소의 main 브랜치에 commit하고 push해서 GitHub Pages에 배포해줘.

github_token.txt의 token="" 값은 인증에만 사용하고 출력하거나 commit하지 마.
AORR.md, MEMORY.md, AORR_LOG.md, github_token.txt, env_settings.txt는 배포에서 제외해.

완료 후 commit hash와 배포 URL만 알려줘.
```
